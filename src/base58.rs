...
// Converts `data` to a base-58 string, that can be decoded by base58_decode().
pub fn base58_encode(data: &[u8]) -> String {
    ...
    while !n.is_zero() {
        ...
        result.push_char(BASE58_ALPHABET.char_at(digit.to_uint().unwrap()));
        n = rest_of_n;
    }
...
// bytes.
pub fn base58_decode(string: &str) -> Vec<u8> {
    ...
    for digit in string.chars().rev() {
        let digit = BASE58_ALPHABET.chars().position(|ch| ch == digit ).unwrap();
...
#[cfg(test)]
mod tests {
    use super::{base58_encode,base58_decode};

    #[test]
    fn test_base58_encode_bitcoin_addresses() {
        ...
        assert_eq!(base58_encode(data).as_slice(), "1Bzd3YTSDwdFfAhMwYNV6A3K5hwYHbaUeG");
        ...
        assert_eq!(base58_encode(data).as_slice(), "112gHKoeKQ3PEXEdAZeC5tBoonPR2UCQot");
        ...
        assert_eq!(base58_encode(data).as_slice(), "111Ai6JPjhcuWxu6ULnRtk34cEj2ZJXfa");
    }

    #[test]
    fn test_base58_encode_out_of_bounds() {
    ...
    #[test]
    fn test_base58_encode_zeroes() {
        let data = [0x00,0x00,0x00,0x00];
        assert_eq!(base58_encode(data).as_slice(), "1111");
    }

    #[test]
    fn test_base58_decode_bitcoin_addresses() {
        // Normal Bitcoin address.
        let data: &[u8] = &[0x00u8,0x78,0x97,0x0e,0x37,0xa7,0xa4,0x71,0xdc,0x33,0xda,0xdb,
                            0x51,0x42,0x06,0x84,0x31,0xb4,0x85,0x17,0xab,0x00,0xed,0xb1,0x45];
        assert_eq!(base58_decode("1Bzd3YTSDwdFfAhMwYNV6A3K5hwYHbaUeG").as_slice(), data);

        // Bitcoin address that starts with two 1's.
        let data: &[u8] = &[0x00,0x00,0x51,0x34,0xda,0xfd,0x60,0x2d,0xcc,0x55,0x36,0x87,
                            0x06,0xd7,0x56,0xc7,0x4f,0xb7,0x74,0x48,0x31,0xf3,0x22,0xdc,0xe3];
        assert_eq!(base58_decode("112gHKoeKQ3PEXEdAZeC5tBoonPR2UCQot").as_slice(), data);

        // Bitcoin address that starts with three 1's (took a while to find
        // this one).
        let data: &[u8] = &[0x00,0x00,0x00,0x23,0xc5,0x36,0xed,0x86,0x7d,0x66,0xa0,0x6b,
                            0x5a,0xfe,0x67,0x5d,0xe8,0xcb,0xe9,0x03,0x94,0x33,0xe5,0x57,0x3d];
        assert_eq!(base58_decode("111Ai6JPjhcuWxu6ULnRtk34cEj2ZJXfa").as_slice(), data);
    }

    #[test]
    fn test_base58_decode_zeroes() {
        let data: &[u8] = &[0x00,0x00,0x00,0x00];
        assert_eq!(base58_decode("1111").as_slice(), data);
    }
}
